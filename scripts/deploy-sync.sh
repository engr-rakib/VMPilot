...
#
#            Drops a per-VM config file into deploy/<env>/ → this script
#            notices it and deploys that ONE VM (other VMs untouched).
#
...
# Usage
# ───────────────────────────────────────────────────────────────────────────
#   ./scripts/deploy-sync.sh <env>                 # scan + deploy new/changed
#   ./scripts/deploy-sync.sh <env> --plan          # dry-run (plan each)
#   ./scripts/deploy-sync.sh <env> --list          # only show the diff table
# ===========================================================================
...
for f in "${ENV_DIR}"/vm-*.tfvars; do
  [ -f "$f" ] || continue
  key="$(grep -E '^[[:space:]]+[a-zA-Z0-9_-]+[[:space:]]*=[[:space:]]*\{' "$f" \
        | head -n1 | sed -E 's/^[[:space:]]*([a-zA-Z0-9_-]+).*/\1/')"
  host="$(grep -E '^[[:space:]]*hostname[[:space:]]*=' "$f" \
          | head -n1 | sed -E 's/.*"[^"]*"/\0/;s/.*"([^"]+)".*/\1/')"
  ip="$(grep -E '^[[:space:]]*ip_address[[:space:]]*=' "$f" \
...
echo ""
echo "=========================================================="
echo "  Sync scan: deploy/${ENV}/  (mode: ${MODE})"
echo "=========================================================="
...
for vm in $(tracked_vm); do
  if [ ! -f "${ENV_DIR}/vm-${vm}_"*.tfvars ] && [ -z "${CFG_KEY[$vm]:-}" ]; then
...
for host in "${!CFG_KEY[@]}"; do
  ...
  if [ "$MODE" = "plan" ]; then
    bash "${SCRIPT_DIR}/deploy-vm.sh" "$ENV" "$host" --plan || true
  else
    bash "${SCRIPT_DIR}/deploy-vm.sh" "$ENV" "$host"
  fi
done
